...
- ✅ **正交标签与权重优化**: 
  - 标签专注于 **路径效率 (Path Efficiency)**：单位回撤下的收益产出
  - 权重专注于 **收益显著性 (Return Significance)**：将 Loss 集中在大机会样本
- ✅ **全方位因子体系 (130+ 因子)**: 
  - 62个技术指标因子
  - 21个K线形态因子
  - 30+ 基本面因子 (PE/PB/ROE/PEG 等)
  - 10+ 市场情绪因子 (全市场涨跌比/换手率等)
- ✅ **因子缓存机制**: 自动缓存计算结果，支持增量更新与特征对齐
- ✅ **多种ML模型**: 支持 XGBoost (回归/软标签)、LightGBM (LambdaRank)
- ✅ **自动特征工程**: 自动计算、标准化与横截面排名归一化
- ✅ **因子重要性分析**: 识别最有预测力的因子，生成审计报告
- ✅ **模型集成**: 组合多个模型提高预测稳定性
- ✅ **混合策略**: 结合 ML 和传统技术分析
- ✅ **回测集成**: 可直接用于回测系统
...
**总计: 130+ 个量化因子 (技术指标 + K线形态 + 基本面 + 情绪)**

## 核心业务逻辑：正交优化 (Orthogonal Optimization)

选股模型的目标是：**在尽量短的时间、用尽量平稳的路径、获得尽量高的收益。**

为了实现这一目标，我们将训练目标拆分为两个相互正交的维度：

1. **Label (Path Efficiency)**: 
   - 关注“路径质量”。
   - 公式: $Return / (1 + \text{Max Drawdown} \times \text{Punish})$。
   - 效果: 相同收益下，回撤越小得分越高；相同回撤下，收益越高得分越高。模型专注于学习“最顺滑”的走势特征。

2. **Weight (Return Significance)**:
   - 关注“机会大小”。
   - 公式: $\ln(1 + |Return| / Volatility)$。
   - 效果: 将模型的注意力集中在那些具有巨大盈利或亏损潜力的样本上，忽略市场杂音。
