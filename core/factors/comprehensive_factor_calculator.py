...
class ComprehensiveFactorCalculator:
    ...
    def calculate_all_factors(self, code: str, data: pd.DataFrame, 
        ...
        base_factors = self._calculate_base_factors(code, data)
    ...
    def _calculate_base_factors(self, code: str, data: pd.DataFrame) -> pd.DataFrame:
        ...
        with warnings.catch_warnings():
            ...
            # D. 高级特征 (时间序列、风险)
            # 这里的实现假设 calculate_..._features 返回的是 Dict[str, float] 的最新值
            # 我们需要将其转换为 Time Series
            try:
                ...
                combined_adv = {**ts_price, **ts_vol, **ts_mom, **risk}
            except Exception:
                combined_adv = {}

            if combined_adv:
                adv_factors = pd.DataFrame([combined_adv] * len(data), index=data.index)
                adv_factors = adv_factors.apply(pd.to_numeric, errors='coerce')
            else:
                adv_factors = pd.DataFrame(index=data.index)
                
            # 合并所有
            factors_list = [tech_factors, candlestick_factors]
