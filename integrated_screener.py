...
class IntegratedScreener:
    ...
    两大核心模型：
    1. SMC流动性猎取模型 - 识别机构建仓完成的瞬间
    2. 威科夫Spring模型 - 捕捉底部反转的最后机会
    """
    ...
    def run_model_3_wyckoff_spring(self, stock_codes, top_n=TOP_N):
        """
        模型三：威科夫Spring反转模型
        ...
        """
        print("\n" + "=" * 60)
        print("模型三：威科夫Spring反转模型")
        print("=" * 60)
        print("策略特点：利用40年K线统计数据的概率优势")
        print(f"分析股票数: {len(stock_codes)}")
        print("-" * 60)
    ...
    def run_comprehensive_screening(self, model='all', top_n=TOP_N):
        ...
        print("=" * 80)
        print(f"基于《全球顶级交易策略与K线结构化趋势识别深度研究报告》")
        ...
        if model in ['all', 'wyckoff']:
            model3_results = self.run_model_3_wyckoff_spring(candidates, top_n)
            all_results['wyckoff_spring'] = model3_results
    ...
    def generate_comprehensive_report(self, all_results):
                ...
                if results:
                    for result in results:
                        ...
                        if code not in stock_counts:
                            stock_counts[code] = []
                        stock_counts[code].append(model_name)
            
            multi_model_stocks = {k: v for k, v in stock_counts.items() if len(v) > 1}
            
            if multi_model_stocks:
                for stock_code, models in multi_model_stocks.items():
                    print(f"  ★ {stock_code}: 出现在 {len(models)} 个模型中")
        ...
        for model_name, results in all_results.items():
            if results:
                filename = f"screening_{model_name}_{timestamp}.csv"
                self._save_results_to_csv(results, filename)
                print(f"\n✓ {model_name} 结果已保存到: {filename}")
...
def main():
    ...
    print("=" * 80)
    print("\n请选择筛选模型:")
    print("1. 模型一：SMC流动性猎取（快速反弹，高胜率）")
    print("2. 模型二：威科夫Spring（底部反转，统计优势）")
    print("3. 综合筛选（运行所有模型）")
    print("0. 退出")
    ...
    try:
        ...
        top_n = input("每个模型返回多少只股票？(默认100000): ").strip()
